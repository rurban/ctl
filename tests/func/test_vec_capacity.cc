        ...
        for (size_t mode = MODE_DIRECT; mode < MODE_TOTAL; mode++)
        {
            ...
            if (mode == MODE_GROWTH)
            {
                ...
                for (size_t pushes = 0; pushes < size; pushes++)
                {
                    ...
                    LOG("capacity  %zu (0x%lx) vs %zu (0x%lx) (%zu) %s\n", aa.capacity, aa.capacity, a.capacity(),
                        a.capacity(), aa.size, aa.capacity != a.capacity() ? "FAIL" : "");
            ...
            LOG("uint8 capacity  %zu (0x%lx) vs %zu (0x%lx)\n", aa.capacity, aa.capacity, a.capacity(), a.capacity());
