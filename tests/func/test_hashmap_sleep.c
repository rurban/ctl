...
// degenerate hash) in the open-addressing <ctl/hashmap.h>.
#include "../test.h"
...
int main(void)
{
    srand(0xbebe);
    const int size = 148;
    hmap_int_int a = hmap_int_int_init_with(broken_hash, int_equal);
    hmap_int_int_reserve(&a, size);
    for (int i = 0; i < size; i++)
    {
        const int vb = TEST_RAND(1000);
        hmap_int_int_insert(&a, vb, vb);
    }
    hmap_int_int_free(&a);
    assert(_slept > 0);
}
