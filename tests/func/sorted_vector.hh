...
template <class K, bool bNoDuplicates = false, class Pr = std::less<K>, class A = std::allocator<K>> class sorted_vector
{
  public:
    ...
    explicit sorted_vector(const Pr &pred = Pr(), const A &al = A()) : key_compare_(pred), vec_(al)
    {
    }
#if (_MSC_VER >= 1300)
    template <class It>
...
#endif
    sorted_vector(const Myt_ &x) : vec_(x.vec_), key_compare_(x.key_compare_)
    {
    }
    ~sorted_vector()
    {
    ...
    iterator insert(iterator it, const value_type &x) // it is the hint
    {
        if (it != end())
        {
            if (bNoDuplicates)
            {
                ...
                {
                    if ((it + 1) == end() || KeyCompare_Gt_(*(it + 1), x))
                    { // use hint
            ...
            {
                if (KeyCompare_Leq_(*it, x) && ((it + 1) == end() || KeyCompare_Geq_(*(it + 1), x)))
                {
                    return InsertImpl_(it + 1, x);
                }
            }
...
#if (_MSC_VER >= 1300)
    template <class It> void insert(It first, It beyond)
    {
...
inline bool operator==(const sorted_vector<K, bNoDuplicates, Pr, A> &x,
...
template <class K, bool bNoDuplicates, class Pr, class A>
inline bool operator<(const sorted_vector<K, bNoDuplicates, Pr, A> &x, const sorted_vector<K, bNoDuplicates, Pr, A> &Y_)
{
    return x.Lt_(Y_);
}
template <class K, bool bNoDuplicates, class Pr, class A>
